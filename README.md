...
| 语汇 | 落在哪 |
| --- | --- |
| **六卷** | 引首（题名）/ 壹 · 琉璃（材质）/ 贰 · 作品 / 叁 · 自述 / 肆 · 手记 / 伍 · 落款 |
| **书耳** | 右侧竖排卷次，兼作阅读进度；鱼尾随滚动下移 |
| **冰裂** | 开卷裂满，滚动愈合（canvas 上按 Voronoi 开片，逐段重绘） |
| **洇墨** | 纸 / 墨两套地色，切换时新色从按钮位置洇开（SVG 湍流位移遮罩 + View Transition，500ms） |
...
## 🎯 第三轮：尺度收敛与动效提速（2026-09-15）

四轮「审美审讯」后落地的一组收敛：把散落的尺寸、时长、文案收成一套能背的数，并删掉一个会抢系统光标的能力。

### 尺度：字号 80 种 → 9 级字阶 + 17 个 display 令牌

全站原本有 80 种字号（多为各组件单独调出来的 clamp 值，看着精心、读起来没系统）。收敛发生在**变量定义处**
（`scroll.css` 的阶梯写成 CSS 变量），clamp 不再算出 80 种；另加 17 个 display 令牌承载标题 / 牌记等少数特例。
`oxlint` 0 警告、build 通过。

### 动效：对标 Material 3 的「>600ms 只给不涉输入的氛围动画」
...
| 洇墨换地色 | 2.4s | **500ms** |
| 数字滚动 | 1.4s | **600ms** |
| 引首搓合（愈合进度） | 约 5 格手势 | **3 格**（`INTRO_BUFFER 0.6→0.1`、`MAX_STEP 0.35→0.5`） |

引首「破镜重圆」仍由人用手感搓、可停半途、可裂回；只是摩擦砍半，不再拦路。

### 自绘毛笔光标：整块删除

原 `lib/cursor.js` 随动笔锋会覆盖 OS 层光标调优、夺 pointer / I-beam 语义，并在首屏与 Voronoi 重算撞车。
已整块删除（CSS + 创建分支），系统箭头原样交还 —— 上方「各处怎么实现的」里已去掉那一行。
（`prefers-reduced-motion` 与触屏下本就安静，不受影响。）

### 文案：首页六卷 736 → 199 字

| 卷 | 改前 | 改后 |
| --- | --- | --- |
| 壹 · 琉璃 | 452 | **109**（材质板 5 件 → 3 件） |
| 首屏主张 | 76 | **0**（姓名即主张） |
| 全站六卷合计 | 736 | **199**（−73%） |

删的是「预告 / 解释 / 自评」三类废话，作品 `tradeoff` 一字未动（那是可核证据）。

### 洇墨收尾残影修复

回流方向（墨 → 纸）的白核半径与半径反推各写一份、比例不一致，合上瞬间最远角差 243px、左下角跳亮 **+10.5%**。
改为 `theme.js` 的 `INK_CORE` 单一来源（JS 的 `R` 与 `Chrome.jsx` 渐变白核停点共用 0.72），两方向最远角亮度差归 **0**
（同坐标 5×5 像素量测实证）。

### 验证

无头 Edge + CDP 真跑：2 视口 × 7 屏裁切 0；翻屏 navtest 每格 `dist=0`；横向溢出 9 宽 × 4 路由 36 组全 0；`devcheck` 实跑 0 错误。
