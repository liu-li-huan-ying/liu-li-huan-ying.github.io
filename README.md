...
| ❄️ **冰裂与愈合（破镜重圆）** | `lib/crackle.js`：按 Voronoi 开片切出裂纹细胞，`heal` 由 0 → 1 逐段合上（由外向内圆拢）。引首（首屏）**分两段**：往下第一滚只播完愈合、**停在引首**，让合上后浮现的那句话（`#healCap`）留得住人看；已合上后再滚，才卷轴翻到「壹 · 琉璃」。回到卷首把冰裂重置为裂满，可再看一次；`prefers-reduced-motion` 下直接跳到愈合态、跳屏瞬移 |
